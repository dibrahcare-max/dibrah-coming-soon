...
export default function AboutPage() {
  return (
    <>
      ...
      {/* ABOUT */}
      <section style={{ padding: '96px 64px', background: 'var(--bg)' }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ fontSize: '.95rem', color: 'var(--muted)', lineHeight: 2 }}>
              شركة سعودية تُعنى بتقديم خدمات الرعاية المنزلية للأطفال وكبار السن، بروحٍ من المسؤولية والاهتمام. كوادر سعودية مؤهلة تجمع بين الخبرة والإنسانية، نوفر خدمات راقية بأيدي سعوديات مدربات، يفهمن احتياجات الأسرة، ويحملن القيم الأصيلة التي تزرع الأمان في كل بيت.
            </p>
            <a href="/book" style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }} className="vision-box">
          {[
            { title: 'الرؤية', text: 'أن تكون دبرة الوجه السعودي الأول في مجال الرعاية وخدمات الحضانة المنزلية.' },
            { title: 'الرسالة', text: 'تمكين الكوادر السعودية من تقديم خدمات رعاية منزلية آمنة واحترافية للأطفال وكبار السن، تعزز جودة الحياة وتدعم استقرار الأسرة.' },
          ].map((c) => (
      ...
      {/* WHY DIBRAH */}
      <section style={{ padding: '96px 64px', background: 'white' }} className="why-section">
        <div style={{ maxWidth: 800, margin: '0 auto 56px', textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: 'clamp(1.8rem,3.5vw,2.8rem)', fontWeight: 900, color: 'var(--dark)', marginBottom: 20 }}>أكثر من مجرد خدمة</h2>
          <p style={{ fontSize: '1.05rem', color: 'var(--muted)', lineHeight: 2.1 }}>
            لأننا لا نُقدّم خدمة فقط؛ نُقدّم <strong style={{ color: 'var(--dark)', fontWeight: 800 }}>سَكينة بيتٍ</strong>، وطمأنينة قلبٍ، وامتدادًا لأيدي الأمهات حين يحتجن من يرعاهنّ ويرعى صغارهنّ. في كل بيتٍ تدخل إليه دِبرة، نغرس الأمان كما تُغرس الرحمة في القلوب — ولسنا مجرد شركة رعاية، بل <strong style={{ color: 'var(--dark)', fontWeight: 800 }}>حضور أنثوي سعودي</strong> يحمل معنى الحنان وعمق المسؤولية ونقاء النية.
          </p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16, marginTop: 48 }} className="pillars-grid">
          {[
            { n: '01', title: 'سَكينة البيت', text: 'حين تغادرين المنزل، نضمن لك أن كل شيء يسير كما لو كنتِ موجودة — بل أفضل. راحة بالك تبدأ من اللحظة التي تختارين فيها دِبرة.' },
            { n: '02', title: 'هوية سعودية أصيلة', text: 'كوادرنا سعوديات يحملن قيم مجتمعنا ويفهمن تفاصيل الأسرة السعودية، مما يجعل التواصل طبيعيًا والثقة سريعة البناء.' },
            ...
            { n: '06', title: 'استمرارية الكادر', text: 'نحرص على أن تتعامل عائلتك مع نفس الكادر باستمرار، لأن بناء علاقة حقيقية بين المرافقة والطفل أو كبير السن هو جوهر خدمتنا.' },
          ].map((p) => (
            <div key={p.n} style={{
              background: 'var(--bg)', borderRadius: 20, padding: '36px 28px',
              border: '1.5px solid rgba(95,97,87,.15)',
              display: 'flex', flexDirection: 'column', gap: 14
            }}>
              <div style={{ fontSize: '.72rem', color: 'rgba(95,97,87,.3)', letterSpacing: '.08em', fontWeight: 700 }}>{p.n}</div>
              <div style={{ fontSize: '1.1rem', fontWeight: 900, color: 'var(--dark)', lineHeight: 1.3 }}>{p.title}</div>
              <p style={{ fontSize: '.88rem', color: 'var(--muted)', lineHeight: 1.8 }}>{p.text}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* STORY */}
      <section style={{ padding: '96px 64px', background: 'var(--bg)' }} className="story-section">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, alignItems: 'center' }} className="story-grid">
          <div>
            <span style={{ fontSize: '3rem', fontWeight: 900, color: '#777C6D', display: 'block', marginBottom: 8 }}>قصتنا</span>
            <h2 style={{ fontSize: 'clamp(1.8rem,3.5vw,2.8rem)', fontWeight: 900, color: 'var(--dark)', lineHeight: 1.1, marginBottom: 20 }}>كيف بدأت دِبرة؟</h2>
            {[
              ...
              'من الرياض، انطلقنا بفريق من السعوديات المؤهلات اللواتي يؤمنّ بأن الرعاية رسالة قبل أن تكون مهنة. واليوم، بعد أن وثق بنا مئات من الأسر، نواصل مسيرتنا بنفس الروح التي بدأنا بها — روح الأمانة والحضور الحقيقي.',
              ...
              <p key={i} style={{ fontSize: '.95rem', color: 'var(--muted)', lineHeight: 2.1, marginBottom: 16 }}>{t}</p>
            ))}
          </div>
      ...
      {/* NEWSLETTER */}
      <div style={{ background: 'var(--dark)', padding: '72px 64px', textAlign: 'center' }} className="nl-section">
        <h2 style={{ fontSize: '1.8rem', fontWeight: 900, color: 'var(--gold)', marginBottom: 8 }}>إشتركي في نشرتنا</h2>
        ...
        <div style={{ display: 'flex', gap: 8, maxWidth: 440, margin: '0 auto' }} className="nl-form">
          <input type="email" placeholder="بريدك الإلكتروني" style={{
            flex: 1, background: 'rgba(227,238,213,.1)', border: '1.5px solid rgba(227,238,213,.2)',
            borderRadius: 8, padding: '12px 16px', fontFamily: 'inherit', color: 'var(--bg)',
            fontSize: '.9rem', outline: 'none'
          }} />
          <button style={{
            background: '#777C6D', color: '#F6F0D7', border: 'none', borderRadius: 8,
            padding: '12px 24px', fontFamily: 'inherit', fontWeight: 800, fontSize: '.88rem',
