...
export default function Nav() {
  ...
  return (
    <>
      <nav style={{
        position: 'sticky', top: 0, right: 0, left: 0, zIndex: 500,
        background: '#EEEEEE',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <a href="#" style={{ fontSize: '.82rem', fontWeight: 700, color: 'var(--dark)', border: '1.5px solid var(--dark)', padding: '8px 20px', borderRadius: 6, transition: 'all .2s' }} className="btn-outline-nav">اهديها</a>
          <a href="/book" style={{ fontSize: '.82rem', fontWeight: 700, color: '#F6F0D7', background: 'var(--dark)', padding: '8px 20px', borderRadius: 6, transition: 'all .2s' }} className="btn-book-nav">احجز الآن</a>
          <a href="#" style={{ fontSize: '.78rem', fontWeight: 700, color: 'var(--dark)', border: '1.5px solid var(--dark)', padding: '7px 14px', borderRadius: 6, display: 'none' }} className="btn-gift-mob">اهديها</a>
      ...
      {mobOpen && (
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
            <a href="#" onClick={() => setMobOpen(false)}
              style={{ background: 'transparent', color: 'var(--dark)', border: '1.5px solid var(--dark)', padding: '11px 28px', borderRadius: 8, fontWeight: 700, fontSize: '.9rem' }}
            >اهديها</a>
            <a href="/book" onClick={() => setMobOpen(false)}
              style={{ background: 'var(--dark)', color: 'var(--bg)', padding: '11px 28px', borderRadius: 8, fontWeight: 700, fontSize: '.9rem' }}
            >احجز الآن</a>
          </div>
        </div>
      )}
      ...
      <style>{`
        @media (max-width: 1024px) {
          ...
          .btn-gift-mob { display: inline-block !important; }
