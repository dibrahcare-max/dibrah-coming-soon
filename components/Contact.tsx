  ...
  return (
    <section id="contact" style={{ padding: '96px 64px', background: 'var(--bg)' }} className="sec-contact">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, marginTop: 56 }} className="rv contact-grid">
        ...
        {/* فورم */}
        <div className='contact-form' style={{ background: '#EEEEEE', borderRadius: 16, padding: 32, border: '1px solid var(--border)' }}>
          ...
          <button style={{ width: '100%', background: 'var(--dark)', color: '#F6F0D7', border: 'none', padding: 13, borderRadius: 8, fontFamily: 'Tajawal, sans-serif', fontWeight: 800, fontSize: '.88rem', cursor: 'pointer' }}>
            إرسال الرسالة
          </button>
        </div>
      </div>

      <style>{`@media (max-width: 1024px) { .contact-grid { grid-template-columns: 1fr !important; gap: 40px !important; } .sec-contact { padding: 64px 24px !important; } } .contact-form input::placeholder, .contact-form textarea::placeholder { color: #F6F0D7; }`}</style>
    </section>
  )
}
