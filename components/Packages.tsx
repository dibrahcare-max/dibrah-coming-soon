const packages = [
  { name: 'الباقة اليومية', sub: 'يوم واحد', featured: false, rows: [{ h: '4 ساعات', v: '350 ﷼' }, { h: '8 ساعات', v: '600 ﷼' }], link: 'https://app.dibrah.net/plans/1/card-checkout' },
  { name: 'الباقة الأسبوعية', sub: 'خمسة أيام', featured: true, rows: [{ h: '4 ساعات', v: '2,000 ﷼' }, { h: '8 ساعات', v: '2,800 ﷼' }], link: 'https://app.dibrah.net/plans/2/card-checkout' },
  { name: 'الباقة الشهرية', sub: 'ستة وعشرون يوم', featured: false, rows: [{ h: '4 ساعات', v: '7,000 ﷼' }, { h: '8 ساعات', v: '14,000 ﷼' }], link: 'https://app.dibrah.net/plans/3/card-checkout' },
]
  ...
  return (
    <section id="packages" style={{ padding: '96px 64px', background: 'var(--bg)' }} className="sec-pkg">
      <div style={{ textAlign: 'center', maxWidth: 640, margin: '0 auto 56px' }} className="rv">
        ...
        <p style={{ fontSize: '.95rem', color: 'var(--muted)', lineHeight: 1.8, marginTop: 10 }}>أسعار شفافة بدون رسوم خفية، مع ضمان استمرارية الكادر المناسب لعائلتك.</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 20 }} className="pkg-grid">
        {packages.map(p => (
          <div key={p.name} className="rv pkg-card" style={{
            background: 'white', borderRadius: 16, padding: '32px 28px', position: 'relative',
            border: p.featured ? '2px solid var(--gold)' : '1.5px solid var(--border)',
            transition: 'all .3s',
          }}>
            {p.featured && (
              <div style={{ position: 'absolute', top: -13, right: 24, background: 'var(--gold)', color: 'white', fontSize: '.65rem', fontWeight: 800, letterSpacing: '.1em', padding: '4px 14px', borderRadius: 20, whiteSpace: 'nowrap' }}>
                الأكثر طلباً
              </div>
            )}
            ...
            {p.rows.map(r => (
              <div key={r.h} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 0', borderBottom: '1px solid rgba(45,58,30,.05)' }}>
                ...
                <span style={{ fontSize: '1rem', fontWeight: 800, color: 'var(--dark)' }}>{r.v}</span>
              </div>
            ))}
            <a href="#" target="_blank" style={{
              display: 'block', width: '100%', marginTop: 20,
              background: p.featured ? 'var(--gold)' : 'var(--dark)',
              color: p.featured ? 'white' : 'var(--bg)',
      ...
      <style>{`
        ...
        @media (max-width: 1024px) { .pkg-grid { grid-template-columns: 1fr !important; max-width: 400px; margin: 0 auto; } .sec-pkg { padding: 64px 24px !important; } }
      `}</style>
    </section>
  )
}
