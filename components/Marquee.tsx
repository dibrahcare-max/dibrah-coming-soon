  ...
  return (
    <div style={{ background: 'var(--gold)', padding: '13px 0', overflow: 'hidden', whiteSpace: 'nowrap' }}>
      <div className="mq-inner" style={{ display: 'inline-flex', gap: 48 }}>
        {doubled.map((item, i) => (
          <span key={i} style={{ fontSize: '.72rem', fontWeight: 800, letterSpacing: '.25em', textTransform: 'uppercase', color: 'white' }}>
            {item}
            {i < doubled.length - 1 && <span style={{ color: 'rgba(255,255,255,.4)', marginRight: 48 }}>✦</span>}
          </span>
        ))}
      </div>
    </div>
